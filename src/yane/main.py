...
def downloadmock(args):
    ...
    gateshead = args.i_mock #Gene information files path
    taxid = args.p_taxa #TaxaID of mock community
    my = args.p_dl_period #Time period of data retrieval
    jobs = args.p_jobs #Jobs (default = 5)
    threads = args.p_threads #Threads (default = 4)
    # endregion
    ...
    ## End process message
    print("\nCheers! ;)")
    print("#### Successfully downloaded mock community from NCBI database ####")
    # endregion
...
def processprimer(args):
    ...
    # region Assign parsed arguments from command flags to python variables
    gateshead = args.i_mock #Mock community Entrez query file path
    lowood = args.i_primer #Primer sequence file path
    thornfield = args.o_dir #Output directory path
    jobs = args.p_jobs #Jobs (default = 5)
    threads = args.p_threads #Threads (default = 4)
    identity = args.p_identity #Primer matching identity (default = 0.8)
    hmplm = args.p_homopolymer #Homopolymer (default = 8)
    degen = args.p_degenerate #Degenerate base (default = 5)
    prilab = args.p_label #Primer labels for customized parameters
    observe_len = args.p_observe_length #Observe length before filtering (default = no)
    level = args.p_length_stat_level #Length statistics taxonomic level (default = Class)
    # endregion
...
#=======================================================================================================================
    ...
    if type(identity) == float:
        ...
        ## End process message
        print("\nCheers! ;)")
        print("#### Successfully extracted and dereplicated hypervariable regions from primer pairs. ####")
        # endregion
...
#=======================================================================================================================
    ...
    if type(degen) == int and type(hmplm) == int:
        ...
        ## End process message
        print("\nCheers! :)")
        print("#### Successfully filtered homopolymers and degenerate bases and dereplicated the datasets. ####")
        # endregion
...
#=======================================================================================================================
    ...
    if observe_len == "no":
        ...
        ## End process message
        print("\nCheers! ;)")
        print("#### Successfully filtered sequence length and dereplicated the datasets. The primer datasets are now cleaned and ready for primer evaluation and classifier training. ####")
        # endregion
...
def customlength(args):
    ...
    if minlen!=0 and maxlen!=0 and taxlen=="" and len(minlen)==len(maxlen) and len(maxlen)==len(prilab):
        ...
        ## End process message
        print("\nCheers! ;)") 
        print("#### >>> Successfully filtered sequence length and dereplicated the datasets: the primer datasets are cleaned and ready for evaluation and classifier training ####")
        # endregion
...
def evaluateprimer(args):
    ...
    # region Assign parsed arguments from command flags to python variables
    gateshead = args.i_mock #Gene information files path
    lowood = args.i_primer #Primer information files path
    thornfield = args.i_dir #Input directory path
    themoor = args.o_dir #Output directory path
    target = args.i_target_taxa #Target taxa tabel file
    level = args.p_tax_level #Taxonomic levels grouped for specificity results
...
def main():
    ...
    # Def#1 Download NCBI ==============================================================================================
    parser_dl = subparsers.add_parser("downloadmock", help="Download NCBI data")
    ## Required arguments
    parser_dl.add_argument("--i-mock", type=str, required=True, help="NCBI query (mock communities before primer extraction) file path [required]")
    parser_dl.add_argument("--p-taxa", type=str, required=True, help="Taxa label or TaxaID of mock communities [required]")
    parser_dl.add_argument("--p-dl-period", type=str, required=True, help="Time period of NCBI data retrieval [required]")
    ## Optional arguments
    ...
    # Def#2 Process NCBI ===============================================================================================
    parser_pc = subparsers.add_parser("processprimer", help="Process NCBI data")
    ## Required arguments
    parser_pc.add_argument("--i-mock", type=str, required=True, help="NCBI query (mock communities before primer extraction) file path [required]")
    parser_pc.add_argument("--i-primer", type=str, required=True, help="Primer information file path [required]")
    parser_pc.add_argument("--o-dir", type=str, required=True, help="Output directory path [required]")
    ## Optional arguments
    ...
    parser_pc.add_argument("--p-identity", type=float, default=0.8, nargs = "+", help="Primer matching identity, which can be set for globally for all primer datasets or customized for each primer dataset [decimals from 0 to 1, default = 0.8]")
    parser_pc.add_argument("--p-homopolymer", type=int, default=8, nargs = "+", help="Filtering criteria for homopolymer length, which can be set for globally for all primer datasets or customized for each primer dataset. Sequences with homopolymer lengths equal to or longer than the criteria will be discarded. [integer, default = 8]")
    parser_pc.add_argument("--p-degenerate", type=int, default=5, nargs = "+", help="Filtering criteria for degenerate base length, which can be set for globally for all primer datasets or customized for each primer dataset. Sequences with degenerate base lengths equal to or longer than the criteria will be discarded. [integer, default = 5]")
    parser_pc.add_argument("--p-label", type=str, default="", nargs = "+", help="Primer names of customized parameters for each primer dataset. If customized parameters are used for identity, homopolymer, and degenerate base lengths, the primer names (similar to ones in primer information table) can be specified here, in the same order as their customized parameters.")
    parser_pc.add_argument("--p-observe-length", type=str, default="no", help="Oberve sequence length statistics and distribution before length filtering step or not. [yes or no, default = no]. If no, the program will proceed with default criteria of min length = 1st percentile and max length = 99th percentile. Sequences with length less or more than the criteria will be automatically discarded. If yes, the program will provide statistics, box plot, and histogram to aid your decision on sequence length criteria before the filtering action, then you have to run a new length filtering command [yane.py custom-length] to filter sequence length with your customized parameters.") 
    parser_pc.add_argument("--p-length-stat-level", type=str, default="Class", help="Taxonomic level for sequence length statistics observation [Taxonomic levels: Kingdom, Phylum, Class, Order, Family, Genus, and Species, default = Class]. This parameter provide sequence length statistics grouped by members of a chosen taxonomic levels. For example, the dafault parameter, Class, provide sequence length statistics for different Class in the mock communities. This results aid setting of length filtering criteria specified by taxon.")
    parser_pc.set_defaults(func=processprimer)
    
    # Def#3 Custom length ==============================================================================================
    parser_cl = subparsers.add_parser("customlength", help="Customize sequence length filtering criteria")
    parser_cl.add_argument("--p-label", type=str, required=True, nargs = "+", help="Primer label for custom length filtering")
    parser_cl.add_argument("--p-min-length", default = 0, type = int, nargs = "+", help="Minimum length")
    parser_cl.add_argument("--p-max-length", default = 0, type = int, nargs = "+", help="Maximum length")
    parser_cl.add_argument("--p-fillen-by-taxon", default = "",type = str, nargs = "+", help="Taxa-specific length customization file path")
    parser_cl.set_defaults(func=customlength)

    # Def#4 Evaluate primer ============================================================================================
    parser_ep = subparsers.add_parser("evaluateprimer", help="Evaluate primer datasets")
    ## Required arguments
    parser_ep.add_argument("--i-mock", type=str, required=True, help="NCBI query (mock communities before primer extraction) file path [required]")
    parser_ep.add_argument("--i-primer", type=str, required=True, help="Primer information file path [required]")
    parser_ep.add_argument("--i-dir", type=str, required=True, help="Input directory path [required]")
    parser_ep.add_argument("--o-dir", type=str, required=True, help="Output directory path [required]")   
    parser_ep.add_argument("--i-target-taxa", type=str, required=True, help="Tab-delimited file of target taxa labels [required]")
    ## Default arguments
